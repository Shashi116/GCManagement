...
const STATUS_DOT = {
  ...
  'Waiting For Code': 'bg-yellow-400 shadow-yellow-400/60',
...
const STATUS_TEXT = {
  ...
  'Waiting For Code': 'text-yellow-400',
...
// DeviceCard handles idle devices only.
// Active sessions (Running/Paused/Waiting/Expired) are rendered by SessionCard.
  ...
  return (
    <div className="glass p-4 flex flex-col gap-3 hover:bg-white/[0.07] transition-all duration-200">
      ...
      <div className="flex items-start justify-between">
        <div>
          <p className="font-semibold text-white text-sm">{device.label}</p>
          <p className="text-xs text-white/30 mt-0.5">
            {device.type === 'PS5' ? `PS5 · Cap: ${device.capacity}` : 'Gaming PC'}
          </p>
        </div>
      ...
      {device.status === 'Available' && (
        <button
          onClick={() => onNewSession(device)}
          className="w-full text-xs btn-primary py-2"
        >
          + New Session
        </button>
      )}
